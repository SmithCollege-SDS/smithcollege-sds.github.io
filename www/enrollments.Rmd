...
plan <- ggplot(data = sds, aes(x = term_date, y = number, color = deptt)) +
  ...
  geom_vline(xintercept = 2012.5:2019.5, linetype = 3) + 
...
```{r enrollment_major}
by_major <- sds %>%
  gather(key = major_type, value = subject, Major1, Major2) %>%
  ...
  group_by(crn_term, major) %>%
  summarize(N = n())

major_plot <- ggplot(by_major, aes(x = as.character(crn_term), y = N, fill = major)) + 
  geom_bar(stat = "identity") +
  scale_y_continuous("Total Declared/Intended Majors") + 
  scale_x_discrete("Term") + 
  scale_fill_brewer("Course", palette = "Set2") + 
...
sds %>%
  ...
  filter(subject != "", !grepl("\\[", subject), grepl("^[A-Z]+$", subject)) %>%
  group_by(subject) %>%
  summarize(num_students = n_distinct(ID)) %>%
  mutate(pct = num_students / sum(num_students)) %>%
...
#  filter(number == 220) %>%
  ...
  ggplot(aes(x = grad_year, y = num_students, fill = is_major)) +   
  ...
  scale_x_continuous("Graduation Year", breaks = c(2012:2022)) + 
  scale_fill_smith_d("SDS Major") +   
...
sds %>%
#  filter(isSDS) %>%
  summarize(N = n(), numIntro = sum(isIntro), 
