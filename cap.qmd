...
cap |>
  group_by(unit) |>
  summarize(
    fte_2021 = sum(ifelse(AY == 2021, fte, 0)),
    fte_2025 = sum(ifelse(AY == 2025, fte, 0))
  ) |>
  drop_na(fte_2025) |>
  mutate(
    unit = fct_reorder(unit, fte_2025),
    is_up = fte_2025 > fte_2021
  ) |>
  ggplot(aes(x = unit, y = fte_2021)) +
  geom_point() + 
  geom_segment(
    aes(xend = unit, yend = fte_2025, color = is_up), 
...
context <- tribble(
  ~fte, ~majors_3, ~label,
  18, 8, paste("OLS regression line:\n", round(mod$coefficients[["fte"]], 2), "majors per FTE"),
  9.8, 34, paste0("+", bump, "\nmajors")
)

gap <- mod |>
  broom::augment(newdata = data.frame(fte = 10)) |>
  rename(majors_3 = .fitted)

ggplot(cap_recent, aes(x = fte, y = majors_3)) +
  geom_segment(data = gap, aes(xend = 10, yend = majors_3 + bump), linetype = 3) +
  geom_abline(
    slope = mod$coefficients[["fte"]], 
    intercept = mod$coefficients[["(Intercept)"]] + bump
  ) + 
  geom_point() + 
  ...
  labs(
    ...
    subtitle = "Based on CAP's 2026 annual report. Figures are for 2023-2025."
  )
```
